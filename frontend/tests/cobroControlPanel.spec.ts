...
const stylesOf = (file: string) =>
	stripComments(
		[...read(file).matchAll(/<style[^>]*>([\s\S]*?)<\/style>/g)]
			.map((m) => m[1])
			.join("\n"),
	);
...
const scrollports = (rules: readonly Rule[]) =>
	rules.filter(({ body }) =>
		body
			...
			.some((declaration) =>
				/^overflow(-[xy])?\s*:\s*(auto|scroll)\b/.test(declaration),
			),
	);
...
describe("nothing in the Cobro columns scrolls except the ticket's lines", () => {
	...
	it("scrolls only ticket lines and the touch-sized preset strip in the action band", () => {
		const found = COBRO_FILES.flatMap((file) =>
			scrollports(rulesOf(file)),
		).map((rule) => `${key(rule.file)} ${rule.selector}`);
		// The preset strip is teleported into the fixed action band, outside
		// the payment columns. It keeps the owner's 48px touch targets.
		expect(found.sort()).toEqual(
			[
				"components/pos/payments/PaymentSaleSummary.vue .pay-summary__lines",
				"components/pos/payments/cobro/CobroTenderPad.vue .cobro-pad__presets--band",
			].sort(),
		);
	});
	...
	it("gives no Cobro section a scrollport in the shell's own stylesheet", () => {
		...
		expect(
			sections.length,
			"the cobro block vanished from Payments.vue",
		).toBeGreaterThan(5);
		expect(
	...
	it("keeps the surface itself unscrolled until the tail unfolds", () => {
		const byName = (selector: string) =>
			rulesOf(PAYMENTS).find((rule) => rule.selector === selector)
				?.body ?? "";
...
describe("the pad is the region that gives", () => {
	...
	it("fills the height its column was given", () => {
		...
		const card =
			rules.find((rule) => rule.selector === ".cobro-pad")?.body ?? "";
		expect(card, "the card must not grow past its cell").toMatch(
			/min-height:\s*0/,
		);
		...
		const stretch =
			rules.find((rule) => rule.selector === ".cobro-pad__pad")?.body ??
			"";
	...
	it("hands the slack to the keys instead of to a scrollbar", () => {
		const rules = rulesOf(pad);
		const grid =
			rules.find((rule) => rule.selector.includes(".pay-keypad__grid"))
				?.body ?? "";
		...
		const keyRule =
			rules.find((rule) => rule.selector.includes(".pay-keypad__key"))
				?.body ?? "";
	...
	it("leaves the phone's keypad exactly as it was", () => {
		...
		const key =
			rulesOf(keypad).find((rule) => rule.selector === ".pay-keypad__key")
				?.body ?? "";
...
describe("a method is one line", () => {
	...
	it("gives the row a fixed height, so a longer name cannot grow the column", () => {
		const row =
			rulesOf(methods).find(
				(rule) => rule.selector === ".cobro-methods__row",
			)?.body ?? "";
		expect(row).toMatch(/display:\s*flex/);
		expect(row).toMatch(/height:\s*var\(--reg-touch-min, 44px\)/);
		expect(row, "a column direction is a card again").not.toMatch(
		...
		const name =
			rulesOf(methods).find(
				(rule) => rule.selector === ".cobro-methods__name",
			)?.body ?? "";
	...
	it("draws one row per configured method and no card chrome", () => {
		expect(source()).toMatch(
			/v-for="payment in rows"[\s\S]{0,200}class="cobro-methods__row"/,
		);
	...
	it("is ONE ROW of chips, and tightens only when the SCREEN is short", () => {
		...
		const list =
			rulesOf(methods).find(
				(rule) => rule.selector === ".cobro-methods__list",
			)?.body ?? "";
		expect(list).toMatch(/display:\s*flex/);
		expect(list).toMatch(/flex-wrap:\s*wrap/);
		expect(list, "a stacked grid is the card list again").not.toMatch(
		...
		expect(style).not.toMatch(
			/@media \(max-width[^)]*\)\s*\{[\s\S]*?\.cobro-methods/,
		);
	});
	...
	it("opens Cobro on a BUTTON, so the tablet keyboard stays down", () => {
		...
		expect(
			source(),
			"an amount input here is the keyboard defect",
		).not.toContain('data-pos-keyboard-target="payment-amount"');
		...
		const markup = source()
			.replace(/<!--[\s\S]*?-->/g, "")
			.replace(/\/\*[\s\S]*?\*\//g, "");
		expect(markup, "no text field on the chip row at all").not.toMatch(
			/<input/,
		);
		expect(read(PAYMENTS)).toContain(
	...
	it("emits nothing `PaymentMethods` does not already emit", () => {
		...
		const names = (block: string) =>
			[...block.matchAll(/"([^"]+)"/g)].map((m) => m[1]).sort();
...
describe("the panel is one grid, uniform across its columns", () => {
	it("places every section it renders, so none is auto-placed", () => {
		...
		for (const area of [
			...
			expect(
				...
				new RegExp(
					`\\.payment-sections--cobro[^{]*\\{\\s*grid-area: ${area};`,
				),
			);
		}
	});
	...
	it("animates nothing a compositor cannot do on its own", () => {
		...
		const cobroRules = [
			...
			...rulesOf(PAYMENTS).filter((rule) =>
				rule.selector.includes("--cobro"),
			),
		];
		for (const file of [PAYMENTS, ...COBRO_FILES]) {
			for (const rule of rulesOf(file)) {
				expect(rule.body, `${key(file)} ${rule.selector}`).not.toMatch(
					/transition:\s*all\b/,
				);
			}
		}
