...
| Giro seed version | boat templates carry `template_version` + sha256 content hash, recorded per apply in the `Doco Applied Artifact` ledger; doco dataset registry carries `version` per dataset; boat `seed_manifests.py` pins all of them per giro bundle (first: `abarrotes-comercio-thin`) | abordo preset seeders still unversioned (provision-path half); golden-flow automation is `job` since manifest v4 (unattended runner `scripts/certification/golden_flow_job.py` ledgers `golden-flow` rows; certification requires a fresh success) — provision-from-zero remains `manual` |
