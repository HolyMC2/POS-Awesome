...
def session_drawer_route(pos_profile, mode_of_payment=None):
    ...
    # Which tender is drawer cash: the open shift's immutable snapshot decides
    # (a mid-shift profile edit cannot move it); the live profile only when
    # the actor has no caja shift. Other tenders keep their own accounts and
    # never need an open caja.
    snapshot = shift_route(shift) if shift else None
    drawer_modes = (snapshot or {}).get("cash_modes") or [
        frappe.db.get_value("POS Profile", pos_profile, "posa_cash_mode_of_payment") or "Cash"]
    if mode_of_payment and mode_of_payment not in drawer_modes:
        return None, None
    if shift:
